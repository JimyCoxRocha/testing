...
export class SemillaRepository {
  ...
  async obtenerSemilla(): Promise<string> {
    try {
      return await this.leerSecretManager();
    } catch (err: any) {
  ...
  private async leerSecretManager(): Promise<string> {
    console.log("creamos cliente - enviando peticion");
    ...
    console.log("se obtuvo respuesta de la semilla");
