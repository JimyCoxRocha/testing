...
export class BMPIniciarSesion extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    let vpcLambdaId: string = config.VPC_LAMBDA_ID_VIRGINIA;
    let subnetId01: string = config.SUBNET_ID_01_VIRGINIA;
    let subnetId02: string = config.SUBNET_ID_02_VIRGINIA;
    let subnetId03: string = config.SUBNET_ID_03_VIRGINIA;
    let securityGroupName: string = config.SECURITY_GROUP_NAME_VIRGINIA;
    let securityGroupID: string = config.SECURITY_GROUP_ID_VIRGINIA;
    let availabilityZones: string[] = ["us-east-1a", "us-east-1b", "us-east-1c"];



    if (config.REGION === "us-west-2") {
      availabilityZones= ["us-west-2a", "us-west-2b", "us-west-2c"];
      vpcLambdaId = config.VPC_LAMBDA_ID_OREGON;
      subnetId01 = config.SUBNET_ID_01_OREGON;
      subnetId02 = config.SUBNET_ID_02_OREGON;
      subnetId03 = config.SUBNET_ID_03_OREGON;
      securityGroupName = config.SECURITY_GROUP_NAME_OREGON;
      securityGroupID = config.SECURITY_GROUP_ID_OREGON;
      console.log("Se asignan valor en variables de la region: ", config.REGION);
    }


    const tablaSesionUsuarioArn = config.TABLA_SESION_USUARIO_ARN.replace("{{region}}", config.REGION);
    const arnIniciarSesion = config.ARN_INICIARSESION.replace("{{region}}", config.REGION);
    const tablaHashUsuarioArn = config.TABLA_HASH_USUARIO_ARN.replace("{{region}}", config.REGION);
    ...
    const lambdaRole = new iam.Role(this, `MyRole`, {
      roleName: `${this.stackName}-rolsg-stack`,
    ...
    const dynamoPolicy = new iam.PolicyStatement({
      ...
      resources: [tablaSesionUsuarioArn, tablaHashUsuarioArn]
    });
    ...
    const subnets = subnetIdsLambdas_.map((subnetId) =>
      ec2.Subnet.fromSubnetId(this, subnetId, subnetId)
    );

    const securityGroup = ec2.SecurityGroup.fromSecurityGroupId(this, securityGroupName, securityGroupID);

    const stepFunctionPolicy = new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
      actions: ['states:StartSyncExecution'],
      resources: [arnIniciarSesion]
    });
    ...
    const encadenamientodb = new dynamodb.Table(this, `encadenamientodb`, {
    ...
    const lambdaProps: NodejsFunctionProps = {
      ...
      logRetentionRetryOptions: {maxRetries: 10, base: cdk.Duration.seconds(5)},
      role: lambdaRole,
      vpc: vpcLambda,
      vpcSubnets: { subnets },
    ...
    const fnGenerarClientIdLogin = new NodejsFunction(this, `${this.stackName}-fnGenerarClientIdLogin`, {
      ...
      environment: {
        ...
        ARN_INICIARSESION: arnIniciarSesion,
    ...
    // === ENCADENAMIENTO: Lambda fnPredecesorValidarGen (Exterior) ===
    const fnPredecesorValidarGen = new NodejsFunction(this, `${this.stackName}-fnPredecesorValidarGen`, {
      functionName: `${this.stackName}-fnPredecesorValidarGen`,
      description: 'Lambda para validar predecesor en encadenamiento',
      entry: path.join(__dirname, '/../src/functions/fnPredecesor.ts'),
      handler: 'fnPredecesor',
      timeout: cdk.Duration.seconds(timeOutValue),
      environment: {
        NODE_OPTIONS: '--enable-source-maps',
        STAGE: config.STAGE,
    ...
    // === ENCADENAMIENTO: Lambda fnSucesorValidarGen (Exterior) ===
    const fnSucesorValidarGen = new NodejsFunction(this, `${this.stackName}-fnSucesorValidarGen`, {
      functionName: `${this.stackName}-fnSucesorValidarGen`,
      description: 'Lambda para registrar sucesor en encadenamiento',
      entry: path.join(__dirname, '/../src/functions/fnSucesor.ts'),
      handler: 'fnSucesor',
      timeout: cdk.Duration.seconds(timeOutValue),
      environment: {
        NODE_OPTIONS: '--enable-source-maps',
    ...
    const fnAsegurarMensajeEntradaValidarGen = new NodejsFunction(this, `${this.stackName}-fnAsegurarMensajeEntradaValidarGen`, {
      functionName: `${this.stackName}-fnAsegurarMensajeEntradaValidarGen`,
      description: 'Lambda para registrar sucesor en encadenamiento',
      entry: path.join(__dirname, '/../src/functions/fnAsegurarMensajeEntrada.ts'),
      handler: 'fnAsegurarMensajeEntrada',
      timeout: cdk.Duration.seconds(timeOutValue),
      environment: {
        NODE_OPTIONS: '--enable-source-maps',
        STAGE: config.STAGE,
    ...
    const stepFunctionRole = new iam.Role(this, `${this.stackName}-stepfunction-rolsg`, {
      roleName: `${this.stackName}-stepfunction-rolsg`,
      ...
      inlinePolicies: {
       ...
       // Rol para Step Function
    const stepFunctionRoleMonetOtros = new iam.Role(this, `${this.stackName}-stepfunction-seguridad-otros-rolsg`, {
      roleName: `${this.stackName}-stepfunction-seguridad-otros-rolsg`,
      assumedBy: new iam.ServicePrincipal('states.amazonaws.com'),
      inlinePolicies: {
        StepFunctionExecutionPolicy: new iam.PolicyDocument({
          statements: [
    ...
    const stepFunctionRoleEncadenamiento = new iam.Role(this, `${this.stackName}-stepfunction-seguridad-encadenamiento-rolsg`, {
      roleName: `${this.stackName}-stepfunction-encadenamiento-rolsg`,
      assumedBy: new iam.ServicePrincipal('states.amazonaws.com'),
      inlinePolicies: {
        StepFunctionExecutionPolicy: new iam.PolicyDocument({
          statements: [
    ...
    const stepFunctionRolePregSeg = new iam.Role(this, `${this.stackName}-stepfunction-preguntas-seguridad-rolsg`, {
      roleName: `${this.stackName}-stepfunction-preguntas-seguridad-rolsg`,
      assumedBy: new iam.ServicePrincipal('states.amazonaws.com'),
      inlinePolicies: {
        StepFunctionExecutionPolicy: new iam.PolicyDocument({
          statements: [
    ...
    stepFunctionGeneralDefinitionString = stepFunctionGeneralDefinitionString
      ...
      .replace(/arn:aws:lambda:\$\{aws:region\}:\$\{aws:accountId\}:function:fnAsegurarMensajeEntradaValidarGen/g, fnAsegurarMensajeEntradaValidarGen.functionArn)
      .replace(/arn:aws:lambda:\$\{aws:region\}:\$\{aws:accountId\}:function:fnSucesorValidarGen/g, fnSucesorValidarGen.functionArn)
      .replace(/arn:aws:lambda:\$\{aws:region\}:\$\{aws:accountId\}:function:fnPredecesorValidarGen/g, fnPredecesorValidarGen.functionArn)
    ...
    const stepFunctionGenerarPregSegIntegration = new apigateway.AwsIntegration({
      ...
      options: {
        credentialsRole: new iam.Role(this, `${this.stackName}-apigw-generar-pregseg-stepfunction-rolsg`, {
...
#else
{
  ...
  "mensajeSistema": "No se pudo obtener respuesta del Step Function"
    ...
    const stepFunctionValidarPregSegIntegration = new apigateway.AwsIntegration({
      ...
      integrationHttpMethod: 'POST',
      options: {
        credentialsRole: new iam.Role(this, `${this.stackName}-apigw-validar-pregseg-stepfunction-rolsg`, {
          assumedBy: new iam.ServicePrincipal('apigateway.amazonaws.com'),
          inlinePolicies: {
...
#else
{
  ...
  "mensajeSistema": "No se pudo obtener respuesta del Step Function"
    ...
    const stepFunctionIntegration = new apigateway.AwsIntegration({
      ...
      integrationHttpMethod: 'POST',
      options: {
        credentialsRole: new iam.Role(this, `${this.stackName}-apigw-stepfunction-rolsg`, {
          assumedBy: new iam.ServicePrincipal('apigateway.amazonaws.com'),
          inlinePolicies: {
...
#else
{
  ...
  "mensajeSistema": "No se pudo obtener respuesta del Step Function"
    ...
    const stepFunctionObtenerTarifaIntegration = new apigateway.AwsIntegration({
      ...
      integrationHttpMethod: 'POST',
      options: {
        credentialsRole: new iam.Role(this, `${this.stackName}-apigw-obtenertarifa-stepfunction-rolsg`, {
          assumedBy: new iam.ServicePrincipal('apigateway.amazonaws.com'),
          inlinePolicies: {
...
#else
{
  ...
  "mensajeSistema": "No se pudo obtener respuesta del Step Function"
    ...
    const stepFunctionValidarGeneralIntegration = new apigateway.AwsIntegration({
      ...
      integrationHttpMethod: 'POST',
      options: {
        credentialsRole: new iam.Role(this, `${this.stackName}-control-gen-stepfunction-rolsg`, {
          assumedBy: new iam.ServicePrincipal('apigateway.amazonaws.com'),
          inlinePolicies: {
            StepFunctionObtenerTarifaPolicy: new iam.PolicyDocument({
              statements: [
...
#else
{
  ...
  "mensajeSistema": "No se pudo obtener respuesta del Step Function"
    ...
    // Endpoint para obtener agenda de tarjetas
    agendaResource.addResource("obteneragendatarjetas")
    .addMethod(
      'POST',
      stepFunctionValidarGeneralIntegration,
      {
    ...
    const stepFunctionOtpIntegration = new apigateway.AwsIntegration({
      ...
      integrationHttpMethod: 'POST',
      options: {
        credentialsRole: new iam.Role(this, `${this.stackName}-apigw-otp-stepfunction-rolsg`, {
          assumedBy: new iam.ServicePrincipal('apigateway.amazonaws.com'),
          inlinePolicies: {
...
#else
{
  ...
  "mensajeSistema": "No se pudo obtener respuesta del Step Function"
    ...
    const stepFunctionGenerarOtpIntegration = new apigateway.AwsIntegration({
      ...
      integrationHttpMethod: 'POST',
      options: {
        credentialsRole: new iam.Role(this, `${this.stackName}-apigw-generar-otp-stepfunction-rolsg`, {
          assumedBy: new iam.ServicePrincipal('apigateway.amazonaws.com'),
          inlinePolicies: {
...
#else
{
  ...
  "mensajeSistema": "No se pudo obtener respuesta del Step Function"
    ...
    const stepFunctionRegistroDispositivoIntegration = new apigateway.AwsIntegration({
      ...
      integrationHttpMethod: 'POST',
      options: {
        credentialsRole: new iam.Role(this, `${this.stackName}-apigw-registro-dispositivo-stepfunction-rolsg`, {
          assumedBy: new iam.ServicePrincipal('apigateway.amazonaws.com'),
          inlinePolicies: {
...
#else
{
  ...
  "mensajeSistema": "No se pudo obtener respuesta del Step Function"
    ...
    // Agregamos recursos requeridos que se habilitarán a medida que tengamos enpoints con autorización
    const buzondigitalResource = api.root.addResource("buzondigital");
    const clienteResource = api.root.addResource("cliente");
    const configuracionResource = api.root.addResource("configuracion");
    const depositorapidoResource = api.root.addResource("depositorapido");
    const geolocalizacionResource = api.root.addResource("geolocalizacion");
    const pagoserviciosResource = api.root.addResource("pagoservicios");
    ...
    const stepFunctionRetiroSinTarjetaIntegration = new apigateway.AwsIntegration({
      ...
      integrationHttpMethod: 'POST',
      options: {
        credentialsRole: new iam.Role(this, `${this.stackName}-apigw-retiro-sin-tarjeta-stepfunction-rolsg`, {
          assumedBy: new iam.ServicePrincipal('apigateway.amazonaws.com'),
          inlinePolicies: {
...
#else
{
  ...
  "mensajeSistema": "No se pudo obtener respuesta del Step Function de retiro"
    ...
    const stepFunctionSfnMonetOtroIntegration = new apigateway.AwsIntegration({
      ...
      integrationHttpMethod: 'POST',
      options: {
        credentialsRole: new iam.Role(this, `${this.stackName}-apigw-sfn-control-monet-otro-logs-rolsg`, {
          assumedBy: new iam.ServicePrincipal('apigateway.amazonaws.com'),
          inlinePolicies: {
            StepFunctionRetiroSinTarjetaPolicy: new iam.PolicyDocument({
              statements: [
...
#else
{
  ...
  "mensajeSistema": "No se pudo obtener respuesta del Step Function de retiro"
    ...
    const stepFunctionEncadenamientoIntegration = new apigateway.AwsIntegration({
      ...
      integrationHttpMethod: 'POST',
      options: {
        credentialsRole: new iam.Role(this, `${this.stackName}-apigw-sfn-control-encadenamiento-logs-rolsg`, {
          assumedBy: new iam.ServicePrincipal('apigateway.amazonaws.com'),
          inlinePolicies: {
            StepFunctionRetiroSinTarjetaPolicy: new iam.PolicyDocument({
              statements: [
  ...
  "mensajeSistema": "No se pudo obtener respuesta del Step Function de retiro"
    ...
    const serviciosHttpResource = new cdk.CustomResource(this, `${this.stackName}-ServiciosHttpResource`, {
      ...
      properties: {
        ...
        AuthorizerId: interceptorAuthorizer.authorizerId
      }
    });


    serviciosHttpResource.node.addDependency(api);
